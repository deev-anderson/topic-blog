...
const EditStory = () => {
  ...
  useEffect(() => {
    const getStoryInfo = async () => {
      setLoading(true);
      try {
        console.log("id do front teste 222");
        console.log(_id);
        const { data } = await axios.get(`/story/editStory/${_id}`, config);
        setStory(data.data);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data } = await axios.put(`/story/${_id}/edit`, formdata, config);
        ...
        <div className="Inclusive-editStory-page ">
          <form onSubmit={handleSubmit} className="editStory-form">
            ...
            <div className="currentlyImage">
              <div className="absolute">Imagem atual</div>
              <img
